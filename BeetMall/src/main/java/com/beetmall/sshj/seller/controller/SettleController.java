...
public class SettleController {
	...
	public ArrayList<Object> searchingData(HttpServletRequest req,SettleVO vo) {
		...
		if(req.getParameter("selectBtnCheck").equals("날짜")) { 
			...
			if( req.getParameter("selectOption").equals("매출일자")) { // 매출일자일때 totalmoney를 구한다.
				System.out.println("날짜 - 매출일자 들어왔나");
				...
				System.out.println("날짜 - 정산일자 들어왔나");
		...
		if(!req.getParameter("selectBtnCheck").equals("날짜")) {
			...
			if(req.getParameter("selectOption").equals("매출일자")) {
				List<SettleVO> totalOrder = service.totalOrder(vo); // totalMoney, totalRecord
				System.out.println("들어왔나");
				if(totalOrder.size() !=0) { // 값이 1이상 있으면 실행된다.
					
					SettleVO vo2 = totalOrder.get(0);
	...
	@ResponseBody
	public void settle_excelDown(HttpServletRequest req, SettleVO vo) {
		vo.setUserid((String)req.getSession().getAttribute("logId"));
		...
		if(selectBtnCheck.equals("날짜")) {// 버튼이 날짜 기준일때
			...
			if(selectOption.equals("매출일자")) { // 매출일자 기준일때
				...
				for(int index = 0; index < list.size(); index++) {
					...
					SettleVO listData = list.get(index);
